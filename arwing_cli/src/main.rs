...
fn main() {
    ...
    //TODO: Un hard-code these values, allow for ranges of actions
    let interactions: Vec<Interaction> = if matches.get_flag("punished_l_cancels") {
        vec![
            Interaction {
                action: 72,
                from_player: player,
...
fn interactions(interactions: Vec<&String>) -> Vec<Interaction> {
    return interactions
        .chunks(3)
        .map(|interaction| {
